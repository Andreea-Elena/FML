    ...
    <div id="content">
      ...
      <div v-for="post in filtered" :key="post.id" id="list-content">
        <q-card
          ...
          @click="redirectToProfile(post)"
        >
          <q-card-section class="col">
            <q-img
              class="avatar"
              :src="'\\statics\\posts\\picture-'+post.id+'-'+post.idUser+'.jpg'"
              style="width:150px; height:150px"
            />
          </q-card-section>
          <q-card-section class="col-6">
            <div class="text-h6">{{post.title}}</div>
            <div class="text-subtitle1">Content: {{post.content}}</div>
            <div class="text-subtitle1">Date: {{post.publishedAt}}</div>
            <div class="text-subtitle1">Category: {{post.category}}</div>
          </q-card-section>
        </q-card>
      </div>
    </div>
